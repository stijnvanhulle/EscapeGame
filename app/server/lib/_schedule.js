...
function Job(name, job, callback) {
  ...
  // Set scope vars
  var jobName = name && typeof name === 'string'
    ? name
    : '<Anonymous Job ' + (++anonJobCounter) + '>';
  this.job = name && typeof name === 'function'
    ? name
    : job;
  ...
  if (this.job === name) {
    ...
    this.callback = typeof job === 'function'
      ? job
      : false;
  } else {
  ...
  this.cancel = function(reschedule) {
    reschedule = (typeof reschedule == 'boolean')
      ? reschedule
      : false;
...
function scheduleNextRecurrence(rule, job, prevDate, endDate) {
  ...
  var date = (rule instanceof RecurrenceRule)
    ? rule.nextInvocationDate(prevDate)
    : rule.next();
  if (date === null) {
    return null;
  }
...
function scheduleJob() {
  ...
  var name = (arguments.length >= 3 && typeof arguments[0] === 'string')
    ? arguments[0]
    : null;
  var spec = name
    ? arguments[1]
