...
* @Last modified time: 2016-12-28T16:03:28+01:00
...
class Prison extends Component {
  ...
  constructor(props, context) {
    ...
    game.events.on('bomStop', () => {
      this.state.currentBomb.addClass('hide');
  ...
  loadBom = (howLong) => {
    const {daytime, rooms, maxRooms, prison, bombs} = this.state;

    if (bombs.length > 0) {
      const randomBomb = rooms[Math.floor(Math.random() * bombs.length)];
      let bomb = bombs[randomBomb];

      bomb.removeClass('hide');
  ...
  onLoad = () => {
    ...
    let background = prison.find('.prison svg #background');
    ...
    for (var i = 0; i < maxRooms; i++) {
      ...
      if (room) {
        rooms.push({element: room, isDestroyed: false, isBom: false});
      }
    }
