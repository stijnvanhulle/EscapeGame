...
* @Last modified time: 2016-12-05T22:42:59+01:00
...
export const setSuccessAndFail = (item) => {
  ...
  for (var i = 0; i < keys.length; i++) {
    let key = keys[i];
    if (key.toLowerCase().indexOf('create') != -1 || key.toLowerCase().indexOf('load') != -1 || key.toLowerCase().indexOf('get') != -1 || key.toLowerCase().indexOf('add') != -1) {
